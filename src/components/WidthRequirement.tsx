/**
 * @file An easy to use react component for automatically show error on different screen width
 * @author AvdanOS
 * @license GPL-3.0
 */
...
const WidthRequirement = ({ width, redirect }: { width: number; redirect?: string | null }) => {
  ...
  dynamicWidth((updatedWidth) => {
    ...
    if (screen.availHeight > width && mobileAndTabletCheck()) {
      // Rotate device
  ...
  return (
    ...
    >
      <Modal.Title>
        <Chiislate>Width Incompatibility</Chiislate>
      </Modal.Title>
      <Modal.Subtitle>
        <Chiislate>{subtitle}</Chiislate>
      </Modal.Subtitle>
      <Modal.Content>
        <p>
          <Chiislate>{content}</Chiislate>
        </p>
      </Modal.Content>
...
export default WidthRequirement;
