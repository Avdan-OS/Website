...
const Menu = () => {
  ...
  dynamicWidth(width => {
    (width < 700) ? setMobileBar(true) : setMobileBar(false);
  })
  const fireClick = () => {
              ...
              <div id="navcol-1" className="collapse navbar-collapse">
                <ul className="navbar-nav ms-auto">
                  <Tabs value={router.asPath} hideDivider onChange={(route) => router.push(route)}>
                    ...
                    <Tabs.Item label={ useMobileBar ? 'Docs' : 'Documentation'} value="/docs" />
                    { useMobileBar ? null : <Tabs.Item label={ useMobileBar ? '' : 'demo'} value="/demo" />}
                  </Tabs>
                </ul>
              </div>
              <Button
